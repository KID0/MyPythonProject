...
def encodeDataInImage(image, data):
    ...
    # 将需要被隐藏的字符串转换成二进制字符串
    map()：对第二个参数（每一个数据）都用一次第一个参数（函数）
    '''
    bytearray():当source参数为字符串时，encoding参数也必须提供，
    函数将字符串使用str.encode方法转换成字节数组
    ''' 
    if len(binary) > len(image.getdata()) * 4:  
    # 如果不可能编码全部数据（隐藏的字符比图片还要多）， 抛出异常
